...
└──────────────┘                   └──────────┬──────────────────────────┘
            ...
            │ data/files/   │        │ + pgvector     │        │ 仅 Chat（管理页）│
            │ 原文件        │        │ 元数据/切片/向量│        │                  │
            └───────────────┘        └───────────────┘        └─────────────────┘
                                              ▲
                                              │ 本地 Embedding
...
| 模型 | 配置 | 何时调用 | 作用 |
|------|------|----------|------|
| **Embedding（向量）** | 内置本地，**不在管理页配置** | ① 文件入库向量化<br>② 提问时问题向量化 | 语义检索（找相关段落） |
| **Chat（对话）** | 管理页：Key / Base URL / 对话模型 | 检索到相关段落后生成回答 | **只根据检索片段**写答案并标 `[1][2]` |

Chat 走云端 OpenAI 兼容接口；Embedding **不走** `/embeddings`。
...
| `instance_settings` | 实例级 **Chat** 配置（Key、Base URL、对话模型） |
...
- 按扩展名策略切分（`ingest.chunk_text`）：Markdown 按标题/段落；csv/xlsx 保留行；其它滑动窗口。
- 默认窗口（`.env` / `config.py`）：`CHUNK_SIZE=500`、`CHUNK_OVERLAP=80`。

### 4.3 向量化 —— **本地 Embedding**

调用 `embed.embed_passages(pieces)`（`sentence-transformers`，`passage:` 前缀）：

- 模型：`multilingual-e5-small`，维度 **384**（`embed.EMBED_DIM`）
- **不请求**云端 `/embeddings`

**作用：** 把「这段文字的含义」编码成向量，之后用距离找「和问题语义最接近的段落」。  
**注意：** 本地模型未下载/加载失败 → ingest 失败，`files.status=failed`。
### 4.4 写入数据库
...
1. `embed.embed_query(question)` → 本地模型对问题向量化（`query:` 前缀），得到 `qvec`。
...
- **Chat** 配置存在 `instance_settings`（管理页写入），**不在 `.env` 里配 Key**。
- `llm._chat_config()` 每次调用前读 DB；缺任一字段 → `LLM_NOT_CONFIGURED`。
- **Embedding** 本地固定；维度常量 `embed.EMBED_DIM`（384）与 `chunks.embedding` 列一致；换模型/改维度须清材料重 ingest。
- 默认检索参数（`config.py`）：
...
2. **ingest** → 抽文本 → 1～N 个 chunk → **本地 Embedding** → 向量进 `chunks`
3. **提问**「唯一工单号是多少？」
4. **本地 Embedding** 把问题转向量 → pgvector 找到含 `WENDU-TXT-NOTE-6610` 的 chunk（向量 + 关键词）
